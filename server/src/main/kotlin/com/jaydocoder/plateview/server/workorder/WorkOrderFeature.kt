        ...
        authenticate("access-token") {
            route("/work-orders") {
                ...
                get("/search") {
                    call.requireWorkOrderAccess(service)
                    val keyword = call.request.queryParameters["keyword"].orEmpty()
                    call.respond(WorkOrderSearchResponse(service.catalogVersion(), service.search(keyword).map(WorkOrderRecord::toResponse)))
                }
                get("/messages/search") {
                    call.requireWorkOrderAccess(service)
                    val keyword = call.request.queryParameters["keyword"].orEmpty()
...
private suspend fun ApplicationCall.receiveImageUpload(storage: WorkOrderImageStorage): WorkOrderImageUpload {
    ...
    return WorkOrderImageUpload(
        ...
        availability = if (files.isEmpty()) "METADATA_ONLY" else "AVAILABLE",
...
private class WorkOrderImageStorage(private val root: File) {
    ...
    fun save(sourceKey: String, attachmentId: String, files: Map<String, StoredUpload>): Map<String, String> {
        ...
        return files.mapValues { (variant, upload) ->
            ...
            val relative = "${safe(sourceKey)}/${safe(attachmentId)}/$variant.$extension"
...
@Serializable private data class WorkOrderMessageBatchRequest(val sourceKey: String, val sourceName: String, val messages: List<WorkOrderIncomingMessageRequest>) {
    fun toModel() = WorkOrderMessageBatch(sourceKey, sourceName, messages.map(WorkOrderIncomingMessageRequest::toModel))
}
...
@Serializable private data class WorkOrderResponse(
    ...
    val vehicles: List<WorkOrderVehicleResponse>,
)
private fun WorkOrderRecord.toResponse() = WorkOrderResponse(
    ...
    images.map { WorkOrderImageResponse(it.id, it.sha256, it.contentType, it.originalSize, it.previewAvailable, it.thumbnailAvailable, it.availability, it.kind, it.fileName, it.pageCount) },
...
private fun WorkOrderAttachment.toResponse() = WorkOrderAttachmentResponse(
    id, kind, fileName, sha256, contentType, originalSize, previewAvailable, thumbnailAvailable, availability, pageCount,
)
