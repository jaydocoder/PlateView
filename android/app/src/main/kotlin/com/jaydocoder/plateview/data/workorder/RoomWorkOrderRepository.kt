...
private fun WorkOrderDto.toDomain() = WorkOrder(
    ...
    people = people.map { WorkOrderPerson(it.rawLine, it.name, it.identityNumber) },
    images = images.map { WorkOrderImage(it.id, it.sha256, it.contentType, it.originalSize, it.previewAvailable, it.thumbnailAvailable, it.availability, it.kind, it.fileName, it.pageCount, it.sourceQuality) },
    vehicles = vehicles.map { WorkOrderVehicle(it.rawDescription, it.rawPlate, it.normalizedPlate, it.vehicleType) },
    orderYear = orderYear,
